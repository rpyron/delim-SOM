##################################################################################################################
#### Empirical dataset: Aeneus - original GBS data from Pyron et al. 2024 with updated environmental variables ###
##################################################################################################################
...
## Read in sample data
Aeneus_data <- read.csv(file = "Data/aeneus56/aeneus56.csv",
                            row.names = 1,
...
Aeneus_SNP <- process.SNP.data.SOM(vcf.path = "Data/aeneus56/aeneus56.vcf.gz",
...
head(Aeneus_SNP[,1:10])#check the SNPs
dim(Aeneus_SNP) #number of loci: 2512, number of samples: 54


## Harmonize layers
samples_retained <- intersect(rownames(Aeneus_SNP),Aeneus_data$Sample[which(!is.na(Aeneus_data$SVL))])
Aeneus_SNP <- Aeneus_SNP[samples_retained,]
head(Aeneus_SNP[,1:10])#check the SNPs
dim(Aeneus_SNP) #number of loci: 2512, number of samples: 44


## Create spatial dataset with coordinates, elevation, and HUC
Aeneus_spatial <- Aeneus_data[, c("Latitude",
                                  "Longitude",
                                  "Elevation")] #extract coordinates and elevation
rownames(Aeneus_spatial) <- Aeneus_data$Sample #assign rownames
Aeneus_spatial$Elevation <- log(Aeneus_spatial$Elevation) #log Elevation
Aeneus_spatial <- Aeneus_spatial[samples_retained,]
head(Aeneus_spatial)


## Create environmental dataset, one-hot encoding lvl4
env_vars <- grep("^(wc|current|huc|lvl)", names(Aeneus_data), value = TRUE)
Aeneus_environmental <- Aeneus_data[env_vars]
rownames(Aeneus_environmental) <- Aeneus_data$Sample
Aeneus_environmental <- Aeneus_environmental[samples_retained,]
Aeneus_environmental <- make.cols.binary.SOM(dataframe = Aeneus_environmental, #make watershed columns binary
...
Aeneus_environmental <- remove.lowCV.multicollinearity.SOM(Aeneus_environmental, #remove highly correlated and low-variance variables
                                                               CV.threshold = 0.05,
                                                               cor.threshold = 0.7)
head(Aeneus_environmental)


## Create morphological trait dataset
trait_names <- c("SVL", "TL", "AG", "CW", "FL", "HL", "SG", "TW", "TO", "FI", "HW", "ED", "IN", "ES", "ON", "IO", "IC")
log_traits <- log(Aeneus_data[, trait_names])
rownames(log_traits) <- Aeneus_data$Sample
log_traits <- log_traits[samples_retained,]
SVL <- log_traits[, "SVL"]
residuals_mat <- sapply(log_traits[, trait_names[-1]], function(y) resid(lm(y ~ SVL)))
Aeneus_morphology <- cbind(SVL = SVL, residuals_mat)
rownames(Aeneus_morphology) <- samples_retained
head(Aeneus_morphology)
...
SOM_Aeneus_clusters <- clustering.SOM(SOM.output = SOM_Aeneus, 
...
plot.Learning.SOM(SOM_Aeneus_clusters)
plot.Layers.SOM(SOM_Aeneus_clusters)
plot.K.SOM(SOM_Aeneus_clusters)
plot.Model.SOM(SOM_Aeneus_clusters)
plot.Structure.SOM(SOM_Aeneus_clusters)
plot.Map.SOM(SOM.output = SOM_Aeneus_clusters, 
...
plot.Variable.Importance.SOM(SOM_Aeneus_clusters,
                             left.margin = 5.8,
                             bar.label.font.size = 0.4, 
                             importance.threshold = 0.1)

#Variable importance across layers
head(round(sort(SOM_Aeneus_clusters$median_variable_importance[[1]], decreasing = T), 2), 100)
round(sort(SOM_Aeneus_clusters$median_variable_importance[[2]], decreasing = T), 2)
round(sort(SOM_Aeneus_clusters$median_variable_importance[[3]], decreasing = T), 2)
round(sort(SOM_Aeneus_clusters$median_variable_importance[[4]], decreasing = T), 2)
