...
analysis.sample.n.by.population.asymmetric_24 <- c(pop1 = 24, pop2 = 12) #number of individuals per population for asymmetric_24
analysis.sample.n.by.population.asymmetric_16 <- c(pop1 = 16, pop2 = 8) #number of individuals per population for asymmetric_16
analysis.sample.n.by.population.asymmetric_8 <- c(pop1 = 8, pop2 = 4) #number of individuals per population for asymmetric_8
...
## Set main parameters
override <- FALSE #if TRUE rerun VCFs even when intermediate results already exist

SOM.replicates <- 100 #number of SOM replicates
SOM.steps <- 100 #number of SOM training iterations
grid_multipler <- 5
...
expected.structure.replicates <- 20 #expected number of STRUCTURE replicates per VCF and K
...
vcf.metadata.table <- do.call(rbind, lapply(seq_along(vcf.filename.parts), function(vcf.index) {
  ...
  data.frame(file = vcf.file.names[vcf.index],
             ...
             simulation.index = as.integer(current.parts[2]),
...
if (any(is.na(vcf.metadata.table$simulation.index))) stop("At least one VCF has missing parsed simulation-index tag")
if (any(is.na(vcf.metadata.table$mig.index))) stop("At least one VCF has missing parsed simulation-index tag")
...
## Check duplicate simulation/divergence/migration combinations
...
expected.mig.tags <- c("0", "8e-7", "2e-6", "4e-6", "8e-6") #define expected migration-rate filename tags
...
parse.vcf.filename.metadata <- function(vcf.filename) {
  ...
  metadata.table <- data.frame(file = vcf.filename,
                               simulation.index = as.integer(filename.parts[2]),
...
create.samples.to.exclude.for.one.vcf <- function(vcf.file.metadata,
  ...
  excluded.sample.table <- data.frame(file = vcf.file.metadata$file,
                                      simulation.index = vcf.file.metadata$simulation.index,
                                      ...
                                      sample.n.pop1 = sample.n.by.population["pop1"],
                                      sample.n.pop2 = sample.n.by.population["pop2"],
...
create.samples.to.exclude.table <- function(vcf.metadata.table,
                                            ...
                                            analysis.set.name,
                                            sampling.seed = 1) {
  if (!("simulation.group" %in% colnames(vcf.metadata.table))) vcf.metadata.table$simulation.group <- vcf.metadata.table$simulation.index
...
run.sNMF.for.one.vcf <- function(vcf.file.path,
  ...
  suppressWarnings(suppressMessages(utils::capture.output({
    sNMF.project <- LEA::snmf(input.file = "input.geno", K = K.values, repetitions = repetitions, CPU = CPU, entropy = TRUE, ploidy = ploidy, project = "new")
...
## Create function to extract best k from optim_k_summary
extract.best.k.from.summary <- function(optim.k.summary.table) {
  if (is.null(optim.k.summary.table) || nrow(optim.k.summary.table) == 0) return(NA_integer_)
  if ("k.label" %in% colnames(optim.k.summary.table) && all(grepl("^k[0-9]+$", optim.k.summary.table$k.label))) {
    if ("Count" %in% colnames(optim.k.summary.table)) {
      best.row.index <- which.max(optim.k.summary.table$Count)
    } else if ("Proportion" %in% colnames(optim.k.summary.table)) {
      best.row.index <- which.max(optim.k.summary.table$Proportion)
    } else {
      best.row.index <- 1
    }
    best.k <- as.integer(sub("^k", "", optim.k.summary.table$k.label[best.row.index]))
    return(best.k)
  }
  candidate.k.columns <- c("k", "K", "n.clusters", "clusters", "n_clusters")
  for (candidate.k.column in candidate.k.columns) {
    if (candidate.k.column %in% colnames(optim.k.summary.table)) {
      if ("Count" %in% colnames(optim.k.summary.table)) {
        best.row.index <- which.max(optim.k.summary.table$Count)
        return(as.integer(optim.k.summary.table[[candidate.k.column]][best.row.index]))
      }
      if ("Proportion" %in% colnames(optim.k.summary.table)) {
        best.row.index <- which.max(optim.k.summary.table$Proportion)
        return(as.integer(optim.k.summary.table[[candidate.k.column]][best.row.index]))
      }
      return(as.integer(optim.k.summary.table[[candidate.k.column]][1]))
    }
  }
  return(NA_integer_)
}
...
extract.k.proportions.from.summary <- function(optim.k.summary.table, max.k = 4) {
...
## Create function to calculate correct assignment proportion for one k = 2 solution
calculate.correct.assignment.proportion.for.one.k2.solution <- function(individual.cluster.assignments, true.population.vector) {
  if (length(individual.cluster.assignments) != length(true.population.vector)) stop("Length mismatch between individual.cluster.assignments and true.population.vector")
  if (any(is.na(individual.cluster.assignments))) return(NA_real_)
  true.population.vector <- as.character(true.population.vector)
  unique.populations <- sort(unique(true.population.vector))
  if (length(unique.populations) != 2) return(NA_real_)
  true.population.numeric <- ifelse(true.population.vector == unique.populations[1], 1L, 2L)
  direct.assignment.proportion <- mean(individual.cluster.assignments == true.population.numeric)
  swapped.assignment.proportion <- mean(ifelse(individual.cluster.assignments == 1L, 2L, 1L) == true.population.numeric)
  correct.assignment.proportion <- max(direct.assignment.proportion, swapped.assignment.proportion)
  return(correct.assignment.proportion)
}


## Create function to calculate mean correct assignment proportion across retained replicates with k = 2
calculate.mean.correct.assignment.proportion.for.k2.replicates <- function(genind.SOM.object, true.population.vector) {
  if (is.null(genind.SOM.object$cluster_assignment)) stop("genind.SOM.object does not contain cluster_assignment")
  if (is.null(genind.SOM.object$optim_k_vals)) stop("genind.SOM.object does not contain optim_k_vals")
  cluster.assignment.matrix <- genind.SOM.object$cluster_assignment
  if (!is.matrix(cluster.assignment.matrix) && !is.data.frame(cluster.assignment.matrix)) stop("cluster_assignment must be a matrix or data frame")
  cluster.assignment.matrix <- as.matrix(cluster.assignment.matrix)
  optim.k.vals <- as.integer(as.vector(genind.SOM.object$optim_k_vals))
  if (length(optim.k.vals) != ncol(cluster.assignment.matrix)) stop("Length mismatch between optim_k_vals and cluster_assignment columns")
  if (nrow(cluster.assignment.matrix) != length(true.population.vector)) stop("Length mismatch between cluster_assignment rows and true.population.vector")
  k2.replicate.indices <- which(optim.k.vals == 2L)
  if (length(k2.replicate.indices) == 0) return(NA_real_)
  replicate.correct.assignment.proportions <- sapply(k2.replicate.indices, function(replicate.index) {
    calculate.correct.assignment.proportion.for.one.k2.solution(individual.cluster.assignments = cluster.assignment.matrix[, replicate.index], true.population.vector = true.population.vector)
  })
  if (all(is.na(replicate.correct.assignment.proportions))) return(NA_real_)
  mean.correct.assignment.proportion.k2 <- mean(replicate.correct.assignment.proportions, na.rm = TRUE) #mean across retained replicates with k = 2 only
  return(mean.correct.assignment.proportion.k2)
}


## Create function to summarize numeric vector
summarize.numeric.vector <- function(x, summary.function) {
  if (length(x) == 0 || all(is.na(x))) return(NA_real_)
  summary.value <- summary.function(x, na.rm = TRUE)
...
parse.STRUCTURE.file.tag.metadata <- function(file.tags) {
  STRUCTURE.metadata.list <- lapply(file.tags, function(file.tag) {
    ...
    metadata.table <- data.frame(file = file.name,
                                 ...
                                 mig.tag = mig.tag,
                                 mig = mig.value,
                                 stringsAsFactors = FALSE) #create metadata row
    return(metadata.table)
  })
...
create.STRUCTURE.mean.lnprob.binomial.table <- function(analysis.set.name,
  ...
  STRUCTURE.mean.lnprob.by.vcf.table <- cbind(STRUCTURE.mean.lnprob.by.vcf.table, STRUCTURE.metadata.table[, c("file", "simulation.index", "mig.index", "tdiv", "mig.tag", "mig"), drop = FALSE]) #add metadata columns
...
run.SOM.workflow.for.one.vcf <- function(vcf.file.path, vcf.file.metadata, SOM.replicates, SOM.steps, sample.n.per.population = NULL, sampling.group = NULL, sampling.seed = 1) {
  ...
  trained.SOM.object <- train.SOM(genind.object.processed, N.replicates = SOM.replicates, N.steps = SOM.steps, parallel = FALSE, verbose = FALSE, grid.multiplier = grid_multipler) #train SOM on full processed SNP matrix
  genind.SOM.object <- clustering.SOM(trained.SOM.object, verbose = FALSE, save.SOM.results = FALSE, clustering.method = clustering_method_SOM) #cluster full SOM object after QE and TE filtering
  ...
  best.k <- extract.best.k.from.summary(optim.k.summary.table) #extract best k
  k.proportion.vector <- extract.k.proportions.from.summary(optim.k.summary.table, max.k = 4) #extract row-level k proportions
  correct.assignment.proportion.k2 <- calculate.mean.correct.assignment.proportion.for.k2.replicates(genind.SOM.object = genind.SOM.object, true.population.vector = adegenet::pop(genind.object.raw)) #calculate mean correct assignment proportion across retained replicates with k = 2 only
  result.summary.row <- data.frame(file = vcf.file.metadata$file,
                                   ...
                                   best.k = best.k,
                                   proportion.k1 = unname(k.proportion.vector["proportion.k1"]),
                                   proportion.k2 = unname(k.proportion.vector["proportion.k2"]),
                                   proportion.k3 = unname(k.proportion.vector["proportion.k3"]),
                                   proportion.k4 = unname(k.proportion.vector["proportion.k4"]),
                                   correct.assignment.proportion.k2 = correct.assignment.proportion.k2,
                                   status = "ok",
                                   error = NA_character_,
                                   stringsAsFactors = FALSE) #create one-row result summary
  if (nrow(optim.k.summary.table) > 0) {
    optim.k.summary.table <- cbind(vcf.file.metadata[rep(1, nrow(optim.k.summary.table)), c("file", "mig.index", "tdiv", "mig.tag", "mig")],
                                   data.frame(n.ind.raw = rep(n.individuals.raw, nrow(optim.k.summary.table)),
                                              ...
                                              best.k = rep(best.k, nrow(optim.k.summary.table)),
                                              correct.assignment.proportion.k2 = rep(correct.assignment.proportion.k2, nrow(optim.k.summary.table)),
...
create.deNovo.kmeans.summary.row <- function(vcf.file.metadata,
                                             ...
                                             deNovo.kmeans.correct.assignment.proportion.k2 = NA_real_,
  ...
  if (is.null(deNovo.kmeans.result) || is.null(deNovo.kmeans.selection)) {
    ...
    deNovo.kmeans.row <- data.frame(file = vcf.file.metadata$file,
                                    ...
                                    deNovo.kmeans.correct.assignment.proportion.k2 = deNovo.kmeans.correct.assignment.proportion.k2,
                                    deNovo.kmeans.n.samples.processed = n.samples.processed,
                                    deNovo.kmeans.n.loci.processed = n.loci.processed,
                                    deNovo.kmeans.status = deNovo.kmeans.status,
                                    deNovo.kmeans.error = deNovo.kmeans.error,
...
run.deNovo.kmeans.BIC.for.one.vcf <- function(vcf.file.path,
  ...
  deNovo.kmeans.correct.assignment.proportion.k2 <- NA_real_
  if (deNovo.kmeans.selection$selected.K == 2L) {
    deNovo.kmeans.K2.result <- deNovo.kmeans.result$kmeans.results[["K=2"]]
    if (!is.null(deNovo.kmeans.K2.result)) {
      deNovo.kmeans.correct.assignment.proportion.k2 <- calculate.correct.assignment.proportion.for.one.k2.solution(individual.cluster.assignments = deNovo.kmeans.K2.result$cluster, true.population.vector = adegenet::pop(genind.object.raw)) #compare inferred K2 clusters to true populations
    }
  }
...
## Create function to create summary tables
summarize.SOM.results <- function(result.table) {
  successful.result.table <- result.table[result.table$status == "ok" & !is.na(result.table$best.k), ] #keep successful runs only
  rownames(successful.result.table) <- NULL
  best.k.proportion.table <- data.frame(best.k = 1:4, n.files = as.integer(table(factor(successful.result.table$best.k, levels = 1:4))), stringsAsFactors = FALSE) #count files by best k
  best.k.proportion.table$proportion <- if (sum(best.k.proportion.table$n.files) > 0) best.k.proportion.table$n.files / sum(best.k.proportion.table$n.files) else NA_real_
  k2.result.columns <- c("file", "mig.index", "tdiv", "mig.tag", "mig", "fst",
                         "deNovo.kmeans.best.k", "deNovo.kmeans.BIC.K1", "deNovo.kmeans.BIC.K2",
                         "deNovo.kmeans.BIC.drop.K1.to.K2", "deNovo.kmeans.correct.assignment.proportion.k2",
                         "sNMF.best.k", "sNMF.cross.entropy.K1", "sNMF.cross.entropy.K2",
                         "sNMF.cross.entropy.drop.K1.to.K2", "sNMF.status",
                         "proportion.k1", "proportion.k2", "proportion.k3", "proportion.k4",
                         "correct.assignment.proportion.k2") #define k = 2 result columns
  k2.result.columns <- k2.result.columns[k2.result.columns %in% colnames(successful.result.table)] #keep available columns only
  k2.result.table <- successful.result.table[successful.result.table$best.k == 2, k2.result.columns, drop = FALSE] #keep k = 2 rows
  rownames(k2.result.table) <- NULL
  k2.correct.assignment.summary.table <- data.frame(n.k2.files = nrow(k2.result.table),
                                                    mean.correct.assignment.proportion = summarize.numeric.vector(k2.result.table$correct.assignment.proportion.k2, mean),
                                                    median.correct.assignment.proportion = summarize.numeric.vector(k2.result.table$correct.assignment.proportion.k2, median),
                                                    min.correct.assignment.proportion = summarize.numeric.vector(k2.result.table$correct.assignment.proportion.k2, min),
                                                    max.correct.assignment.proportion = summarize.numeric.vector(k2.result.table$correct.assignment.proportion.k2, max),
                                                    stringsAsFactors = FALSE) #summarize k2 assignment accuracy
  return(list(successful.result.table = successful.result.table,
              best.k.proportion.table = best.k.proportion.table,
              k2.result.table = k2.result.table,
              k2.correct.assignment.summary.table = k2.correct.assignment.summary.table))
}
...
samples.to.exclude.asymmetric_8 <- create.samples.to.exclude.table(vcf.metadata.table = vcf.metadata.table,
                                                                   ...
                                                                   sampling.seed = analysis.sample.random.seed)
samples.to.exclude.for.STRUCTURE <- rbind(samples.to.exclude.symmetric_16,
...
rownames(vcf.metadata.table) <- NULL
vcf.metadata.table$simulation.group <- vcf.metadata.table$simulation.index #define matched simulation group across migration rates from simulation number
  ...
  if (!override && check.vcf.intermediate.results.available(vcf.file.metadata = current.metadata, intermediate.results.directory = intermediate.results.directory)) {
    ...
    current.output <- tryCatch({
      ...
      result.summary.row <- data.frame(file = current.metadata$file,
                                       ...
                                       n.loci.processed = NA_integer_,
                                       best.k = NA_integer_,
                                       proportion.k1 = NA_real_,
                                       proportion.k2 = NA_real_,
                                       proportion.k3 = NA_real_,
                                       proportion.k4 = NA_real_,
                                       correct.assignment.proportion.k2 = NA_real_,
                                       status = "error",
...
## Summarize SOM results
summary.output <- summarize.SOM.results(result.table = result.table) #summarize results
successful.result.table <- summary.output$successful.result.table #extract successful result table
best.k.proportion.table <- summary.output$best.k.proportion.table #extract best-k proportions
k2.result.table <- summary.output$k2.result.table #extract k2 result table
k2.correct.assignment.summary.table <- summary.output$k2.correct.assignment.summary.table #extract k2 assignment summary
...
write.csv(best.k.proportion.table, file.path(results.directory, "fastsimcoal2_SOM_best_k_proportions.csv"), row.names = FALSE) #save best-k proportions
write.csv(k2.result.table, file.path(results.directory, "fastsimcoal2_SOM_k2_correct_assignment_per_file.csv"), row.names = FALSE) #save per-file k2 results
write.csv(k2.correct.assignment.summary.table, file.path(results.directory, "fastsimcoal2_SOM_k2_correct_assignment_summary.csv"), row.names = FALSE) #save k2 assignment summary
...
## Save PCA plot and table
pca.file.tag <- tools::file_path_sans_ext(current.pca.metadata$file) #create PCA file tag
write.csv(pca.plot.table, file.path(results.directory, paste0(pca.file.tag, "_PCA_plot_table.csv")), row.names = FALSE) #save PCA plot table
results.directory
