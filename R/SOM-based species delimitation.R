...
## Number of clusters are chosen by k-means clustering of grid cells into proximate units based on weighted sum of squares (WSS) of neighbor distances between adjacent cells
...
setwd("C:/Users/danie/Desktop/PhD research/Hemileuca maia research/Analyses")
...
a <- read.structure("./seal_in_c90.str",
                    ...
                    row.marknames = 0,
                    NA.char = -9)
alleles <- makefreq(missingno(a, type = "loci", cutoff = 0.20))
space <- as.matrix(read.csv("./seal_space.csv", header = T, row.names = 1))
climate <- as.matrix(read.csv("./seal_climate.csv", header = T,row.names = 1))
traits <- as.matrix(read.csv("./seal_traits.csv", header = T, row.names = 1))
rownames(traits) <- rownames(alleles)
...
run.SOM <- function(input_data, #can be one matrix/dataframe or multiple matrices/dataframes provided as list()
                    N.steps = 10, #number of training iterations for SOM
                    N.replicates = 6, #number of SOM runs
                    max.k = 5, #maximum of considered clusters K
                    ...
                    message.N.replicates = 50) #frequency of progress messages during training (message is printed every message.N.replicates iterations)
{
  ...
  if (is.list(input_data) && length(input_data) > 1 && !is.data.frame(input_data)) { 
    ...
    if (any(input_data[[1]] < 0, na.rm = T) || any(input_data[[1]] > 1, na.rm = T)) {
      input_data[[1]] <- (input_data[[1]] - min(input_data[[1]], na.rm = T)) / (max(input_data[[1]], na.rm = T) - min(input_data[[1]], na.rm = T))
      message(input_data_names, " dataset was normalized to range [0, 1]")
    }
  }
  ...
  SOM_output_grid <- somgrid(xdim = round(sqrt(5 * sqrt(length(rownames(input_data[[1]]))))),
                             ydim = round(sqrt(5 * sqrt(length(rownames(input_data[[1]]))))),
                             topo = "hexagonal",
  ...
  replicate_som <- function(j) {
    ...
    if (which(w_vec == min(w_vec)) == 1) {
      num_clusters <- 1
    } else { #choose K by diffNgroup from DAPC
  ...
  # Calculate admixture_matrix summarizing cluster memberships and return it
  admixture_matrix <- t(apply(assignment_matrix, 1, FUN = function(x) {
  ...
  colnames(admixture_matrix) <- paste0("Cluster_", seq_len(ncol(admixture_matrix)))
  ...
  SOM_results <- list(cluster_assignment = cluster_assignment, 
                      ...
                      admixture_matrix = admixture_matrix,
                      optim_k = optim_k,
...
plot.K.SOM <- function(SOM.output, 
                       ...
                       save = F, #option to save plot
                       overwrite = T, #option to overwrite plot
                       type = "svg", #set plot type: "svg", "png", "jpg"
                       ...
                       margin.top = 2,
                       margin.right = 1,
                       title = "Number of clusters (k)") { #main plot title
  ...
  if (save) {
    ...
    boxplot(t(SOM.output$BIC_values), 
            outline = F, 
            notch = F, 
            axes = F, 
            ylab = "BIC", 
            ...
            col = k.cols)
    axis(1, 
         at = 1:SOM.output$max_k, 
         labels = 1:SOM.output$max_k)
    axis(2, 
         at = round(range(unlist(SOM.output$BIC_values))), 
         las = 3)
    title(title, 
          line = 0)
    ...
    abline(h = 0, 
           ...
           col = "black")
    axis(1, 
         at = 1:(SOM.output$max_k - 2), 
         labels = 1:(SOM.output$max_k - 2))
    axis(2, 
         at = sort(c(0, round(range(unlist(d_wss))))), 
         las = 3)
    
    # Create barplot for sampling frequency of cluster counts
    barplot_data <- table(factor(SOM.output$optim_k, levels = 1:SOM.output$max_k)) / SOM.output$N_replicates
...
## Function to plot Structure-like barplots and save admixture proportions as csv file
plot.Structure.SOM <- function(SOM.output, 
                               ...
                               margin.left = 4, 
                               margin.right = 2,
                               sort.by.col = 1, #specify integer giving column index of admixture matrix for ordering rows of admixture matrix (if NULL, no ordering is performed)
                               linkage.method = "single") { #agglomeration method used for hierarchical clustering (see hclust function)
  
  # Ensure admixture_matrix is valid
  if (is.null(SOM.output$admixture_matrix) || !is.matrix(SOM.output$admixture_matrix)) {
    stop("admixture_matrix of SOM.output not valid")
  }
  
  # If there's only one cluster, skip plotting
  if (ncol(SOM.output$admixture_matrix) == 1) { 
    stop("Only one cluster detected - skipping structure plot")
  }
  
  # Order rows of admixture_matrix
  if (!is.null(sort.by.col)) {
    if (sort.by.col > ncol(SOM.output$admixture_matrix)) {
      stop(paste0("sort.by.col exceeds number of columns in admixture_matrix - select number from 1-", ncol(SOM.output$admixture_matrix), " or NULL to prevent sorting"))
    }
    admixture_proportions <- SOM.output$admixture_matrix[order(SOM.output$admixture_matrix[, sort.by.col]), ]
  } else {
    admixture_proportions <- SOM.output$admixture_matrix #no sorting when set as NULL
  }
  
  # Perform hierarchical clustering on distance matrix
  cluster_order <- hclust(dist(admixture_proportions), 
                          method = linkage.method)$order
  SOM_admixture_proportions <- admixture_proportions[cluster_order, ]
  
  # Generate layer colors
  layer_colors <- col.pal(ncol(SOM_admixture_proportions)) 
  ...
  make.structure.plot(admix.proportions = SOM_admixture_proportions, 
                      sample.names = rownames(SOM_admixture_proportions), 
                      mar = c(margin.bottom, 
                              margin.left, 
                              margin.top, 
                              margin.right), 
...
plot.Map.SOM <- function(SOM.output,
  ...
  # Ensure admixture_matrix is valid
  if (is.null(SOM.output$admixture_matrix) || !is.matrix(SOM.output$admixture_matrix)) {
    stop("admixture_matrix of SOM.output is not valid")
  }
  ...
  # Prepare admixture matrix
  q_matrix = as.data.frame(SOM.output$admixture_matrix) #convert admixture_matrix to dataframe
  q_matrix$Ind = rownames(SOM.output$admixture_matrix) #add sample names
  
  # Check if number of rows in admixture matrix matches number of coordinates
  if (nrow(q_matrix) != nrow(Coordinates)) {
    stop("Number of samples in admixture_matrix does not match number of samples in Coordinates")
  }
  
  # Define color palette for pie charts
  k.cols = pie.col.pal(ncol(SOM.output$admixture_matrix))
...
plot.Variable.Importance.SOM <- function(SOM.output, 
  ...
  SOM_results <- list(cluster_assignment = SOM.output$cluster_assignment, 
                      ...
                      admixture_matrix = SOM.output$admixture_matrix,
...
SOM_multi <- plot.Variable.Importance.SOM(SOM_multi)
SOM_multi2 <- plot.Variable.Importance.SOM(SOM_multi2)
SOM_single <- plot.Variable.Importance.SOM(SOM_single)
SOM_single2 <- plot.Variable.Importance.SOM(SOM_single2)
SOM_single3 <- plot.Variable.Importance.SOM(SOM_single3)
