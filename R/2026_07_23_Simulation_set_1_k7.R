...
run.SOM.benchmark <- function(input_data,
  ...
  time_per_simulation <- elapsed_time
...
#### Test effect of codebook learning-rate tuning ##############################

## Set learning-rate tuning options to test
learning_rate_tuning_values <- c(FALSE, TRUE)
...
if (file.exists(sim_results_learning_rate_tuning_csv) && !overwrite) {
  ...
  for (learning_rate_tuning_value in learning_rate_tuning_values) {
    cat("Running for learning-rate tuning:", learning_rate_tuning_value, "\n")
    condition_safe_name <- create.safe.file.name(learning_rate_tuning_value)
    condition_complete_csv <- file.path(intermediate_results_dir, paste0("Sim_results_learning_rate_tuning_", condition_safe_name, "_complete.csv"))
    condition_partial_csv <- file.path(intermediate_results_dir, paste0("Sim_results_learning_rate_tuning_", condition_safe_name, "_partial.csv"))
    condition_partial_rds <- file.path(intermediate_results_dir, paste0("Sim_results_learning_rate_tuning_", condition_safe_name, "_partial.rds"))
    if (file.exists(condition_complete_csv) && !overwrite) {
      stats_df_all <- read.csv(condition_complete_csv)
      message("Loaded completed learning-rate-tuning intermediate results for ", learning_rate_tuning_value)
    } else {
      stats_list <- vector("list", length(simulation_results))
      skipped_simulations <- list()
      condition_skipped_csv <- file.path(intermediate_results_dir, paste0("Sim_results_learning_rate_tuning_", condition_safe_name, "_skipped.csv"))
      for (simulation_index in seq_along(simulation_results)) {
        cat("Testing simulation", simulation_index, "of", length(simulation_results), "for learning_rate_tuning:", learning_rate_tuning_value, "\n")
        flush.console()
        simulation_data <- simulation_results[[simulation_index]]
        som_input <- list(SNP = simulation_data$SNP,
                          Morphology = simulation_data$Morphology,
        ...
        stats_list[[simulation_index]] <- tryCatch({
          run.SOM.benchmark(input_data = som_input,
                            true_labels = simulation_data$Cluster,
                            group_column_name = "learning_rate_tuning",
                            group_value = learning_rate_tuning_value,
                            clustering_method = clustering_method,
                            training_neighborhoods = training_neighborhoods_SOM,
                            max_k = max_k_SOM,
                            learning_rate_tuning_option = learning_rate_tuning_value,
                            sim_stats_row = simulation_data$sim_stats,
                            verbose = verbose_SOM)
        }, error = function(error_message) {
          skip_message <- paste0("Skipped simulation ", simulation_index, " for learning_rate_tuning = ", learning_rate_tuning_value, " because: ", conditionMessage(error_message))
          message(skip_message)
          skipped_simulations[[length(skipped_simulations) + 1]] <<- data.frame(learning_rate_tuning = learning_rate_tuning_value, sim = simulation_index, error_message = conditionMessage(error_message), stringsAsFactors = FALSE)
          NULL
        })
        save.current.condition.results(stats_list = stats_list, condition_partial_csv = condition_partial_csv, condition_partial_rds = condition_partial_rds)
        if (length(skipped_simulations) > 0) write.csv(do.call(rbind, skipped_simulations), condition_skipped_csv, row.names = FALSE)
      ...
      stats_df_all <- bind.results.with.sim(stats_list)
      if (is.null(stats_df_all)) stop("All simulations failed for learning_rate_tuning = ", learning_rate_tuning_value)
      write.csv(stats_df_all, condition_complete_csv, row.names = FALSE)
    }
    if (!is.null(stats_df_all)) all_results_learning_rate_tuning[[as.character(learning_rate_tuning_value)]] <- stats_df_all
    save.partial.results(result_list = all_results_learning_rate_tuning,
                         combined_csv = sim_results_learning_rate_tuning_csv,
                         combined_rds = sim_results_learning_rate_tuning_rds)
  }
  ...
  if (length(all_results_learning_rate_tuning) == 0) stop("No learning-rate-tuning results to summarize")
...
summary_sim_stats_learning_rate_tuning <- full_sim_stats_learning_rate_tuning %>%
  dplyr::group_by(learning_rate_tuning) %>%
  dplyr::summarise(mean_ARI = safe.mean(ARI),
                   mean_K = safe.mean(K_inferred),
                   mean_Acc = safe.mean(Acc),
                   mean_Time = safe.mean(Time[!duplicated(sim)]),
...
## Plot results
learning_rate_tuning_ARI_model_plot <- plot.categorical.gaussian.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                 condition_variable = "learning_rate_tuning",
                                                                                 response_variable = "ARI",
                                                                                 condition_levels = learning_rate_tuning_values,
                                                                                 x_axis_label = "Learning-rate tuning",
                                                                                 y_axis_label = "Adjusted Rand Index",
                                                                                 estimate_color = benchmark_plot_color_ARI)
learning_rate_tuning_Acc_model_plot <- plot.categorical.gaussian.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                 condition_variable = "learning_rate_tuning",
                                                                                 response_variable = "Acc",
                                                                                 condition_levels = learning_rate_tuning_values,
                                                                                 x_axis_label = "Learning-rate tuning",
                                                                                 y_axis_label = "Assignment accuracy",
                                                                                 estimate_color = benchmark_plot_color_Acc)
learning_rate_tuning_K_correct_model_plot <- plot.categorical.binary.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                     condition_variable = "learning_rate_tuning",
                                                                                     response_variable = "K_correct",
                                                                                     condition_levels = learning_rate_tuning_values,
                                                                                     x_axis_label = "Learning-rate tuning",
                                                                                     y_axis_label = "Probability K correct",
                                                                                     estimate_color = benchmark_plot_color_K_correct)
learning_rate_tuning_K_far_off_model_plot <- plot.categorical.binary.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                     condition_variable = "learning_rate_tuning",
                                                                                     response_variable = "K_far_off",
                                                                                     condition_levels = learning_rate_tuning_values,
                                                                                     x_axis_label = "Learning-rate tuning",
                                                                                     y_axis_label = "Probability K far off",
                                                                                     estimate_color = benchmark_plot_color_K_far_off)
learning_rate_tuning_QE_model_plot <- plot.categorical.gaussian.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                condition_variable = "learning_rate_tuning",
                                                                                response_variable = "QE",
                                                                                condition_levels = learning_rate_tuning_values,
                                                                                x_axis_label = "Learning-rate tuning",
                                                                                y_axis_label = "Quantization error",
                                                                                estimate_color = benchmark_plot_color_QE)
learning_rate_tuning_TE_model_plot <- plot.categorical.gaussian.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                condition_variable = "learning_rate_tuning",
                                                                                response_variable = "TE",
                                                                                condition_levels = learning_rate_tuning_values,
                                                                                x_axis_label = "Learning-rate tuning",
                                                                                y_axis_label = "Topographic error",
                                                                                estimate_color = benchmark_plot_color_TE)
learning_rate_tuning_Time_model_plot <- plot.categorical.gaussian.benchmark.model(input_data = full_sim_stats_learning_rate_tuning,
                                                                                  condition_variable = "learning_rate_tuning",
                                                                                  response_variable = "Time",
                                                                                  condition_levels = learning_rate_tuning_values,
                                                                                  x_axis_label = "Learning-rate tuning",
                                                                                  y_axis_label = "Time (seconds)",
                                                                                  estimate_color = benchmark_plot_color_Time)
summary(learning_rate_tuning_ARI_model_plot$model)
summary(learning_rate_tuning_Acc_model_plot$model)
summary(learning_rate_tuning_K_correct_model_plot$model)
summary(learning_rate_tuning_K_far_off_model_plot$model)
summary(learning_rate_tuning_QE_model_plot$model)
summary(learning_rate_tuning_TE_model_plot$model)
summary(learning_rate_tuning_Time_model_plot$model)
learning_rate_tuning_ARI_model_plot$plot
learning_rate_tuning_Acc_model_plot$plot
learning_rate_tuning_K_correct_model_plot$plot
learning_rate_tuning_K_far_off_model_plot$plot
learning_rate_tuning_QE_model_plot$plot
learning_rate_tuning_TE_model_plot$plot
learning_rate_tuning_Time_model_plot$plot
